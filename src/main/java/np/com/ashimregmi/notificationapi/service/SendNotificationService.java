...
public class SendNotificationService implements SendNotificationApi {
    ...
    private final NotificationSenderApi iosNotificationSender;
    @Override
    public void send(SpecificDeviceRmqMessage specificDeviceRmqMessage) {
        if (specificDeviceRmqMessage.device() == Device.ANDROID) {
            androidNotificationSender.send(specificDeviceRmqMessage);
        } else if (specificDeviceRmqMessage.device() == Device.IOS) {
            iosNotificationSender.send(specificDeviceRmqMessage);
        }
    }
}
